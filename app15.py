...
@st.cache_data
def load_data():
    df = pd.read_csv("consumptionai2.csv")  # CSV must be in repo
...
def train_models(df):
    ...
    months = ['May', 'Jun', 'Jul', 'August', 'Sept', 'Oct', 'Nov', 
              'Dec', 'Jan', 'Feb', 'Mar', 'Apr']
    models = {}
...
month = st.selectbox("Select Month", ['May', 'Jun', 'Jul', 'August', 'Sept', 'Oct', 'Nov', 
                                      'Dec', 'Jan', 'Feb', 'Mar', 'Apr'])
...
if connected_load <= 0:
    ...
    if st.button("🔍 Predict Consumption"):
        ...
        # --- New: Interactive Line Graph for all 12 months ---
        all_months = ['May', 'Jun', 'Jul', 'August', 'Sept', 'Oct', 'Nov', 
                      'Dec', 'Jan', 'Feb', 'Mar', 'Apr']
        predictions_all = []
        for m in all_months:
            pred = models[m].predict(input_data)[0]
            predictions_all.append(pred)

        pred_df = pd.DataFrame({
            "Month": all_months,
            "Predicted Consumption (kWh)": predictions_all
        })
        ...
        fig = px.line(
            ...
            markers=True,
            title="Predicted Monthly Consumption Trend"
        )
        fig.update_layout(
            xaxis_title="Month",
